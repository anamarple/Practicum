...
class Table(Canvas):
    ...
    def popupMenu(self, event, rows=None, cols=None, outside=None):
        ...
        defaultactions = {
                        ...
                        # Ana's addition
                        'Previous': lambda: self.change_bucket('Previous'),
                        'General': lambda: self.change_bucket('General'),
                        'Volumetric and Reserves Estimates' : lambda: self.change_bucket('Volumetric and Reserves Estimates'),
                        'Production' : lambda: self.change_bucket('Production'),
                        'Development Plans' : lambda: self.change_bucket('Development Plans'),
                        'Economics' : lambda: self.change_bucket('Economics'),
                        'Field Reports' : lambda: self.change_bucket('Field Reports'),
                        'Seismic Data' : lambda: self.change_bucket('Seismic Data'),
                        'Geologic Maps' : lambda: self.change_bucket('Geologic Maps'),
                        'Bubble Maps' : lambda: self.change_bucket('Bubble Maps'),
                        'PVT and Test Data' : lambda: self.change_bucket('PVT and Test Data'),
                        'Petrophysical Summaries' : lambda: self.change_bucket('Petrophysical Summaries'),
                        'Cross-Sections' : lambda: self.change_bucket('Cross-Sections'),
                        'Logs' : lambda: self.change_bucket('Logs'),
                        'Field Activity' : lambda: self.change_bucket('Field Activity'),
                        'Modeling' : lambda: self.change_bucket('Modeling')}
        ...
        editcommands = ['Previous', 'General', 'Volumetric and Reserves Estimates', 'Production', 'Development Plans',
                        'Economics', 'Field Reports', 'Seismic Data', 'Geologic Maps', 'Bubble Maps', 'PVT and Test Data',
                        'Petrophysical Summaries', 'Cross-Sections', 'Logs', 'Field Activity', 'Modeling']
        ...
        # Ana's addition ***
        createSubMenu(popupmenu, 'Change Bucket2', editcommands)
        createSubMenu(popupmenu, 'Export to csv', exportcommand)
    ...
    # Ana's addition ***
    def update_bucket1(self, bucket):
        if (bucket == 'Economics'):
            return 'Economics'
    ...
    def change_bucket(self, bucket):
        ...
        # Change Bucket2 catgeory to desired bucket
        for row in rows:
            df.iloc[row, 3] = bucket
            df.iloc[row, 2] = self.update_bucket1(bucket)
